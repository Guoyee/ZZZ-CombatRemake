...
- **GA 结束位的第三种情形 —— 蒙太奇自动 blend-out（2026-09-10 补）**：带 BlendOut 的蒙太奇由引擎在**最后一段剩余播放时间 ≤ `BlendOutTriggerTime`（默认 = `BlendOut` 时长）**时自动 `Stop(非打断)` → 先 `OnBlendOut` 再 `OnCompleted`（基类两个回调都 EndAbility，`FAnimMontageInstance::Advance` / `UZZZGameplayAbility::PlayMontage`）。⇒ **没摆 EndEventTag notify 的技能，GA 实际寿命 = 蒙太奇时长 − BlendOut 时长**（招架蒙太奇 0.567−0.25 = 0.317s）。凡"GA 必须活到外部事件到场"的技能（招架等敌人打击帧 `Event.Combat.ParryImpact`；特殊技两段式的 Lead）一律**覆写 `OnMontageBlendOut()` 拦截**（招架 `UZZZAssistDefensive` 2026-09-10、两段式 `UZZZSpecialAttack` 2026-09-03 同手法，覆写经基类 UFUNCTION thunk 虚分派生效）——混合出观感不变，结束位交给 `OnMontageCompleted`。⚠ 与"源动画时长 < 蒙太奇段长 → 段长未到就 blend-out"（§4.9.2，2026-09-05）是**两个独立机制**，勿混。
...
- **双轨兜底**：`ParryPending`/`AttackWindow` 由 E 攻击 GA EndAbility 清理；`CanCombo`/`CanBuffer` 由基类 EndAbility 统一清理（2026-09-05 集中）。定格期间大招架蒙太奇提前 blend-out 的病根 = 源动画实际时长 < 蒙太奇段长（资产问题，2026-09-05 定位；**与 2026-09-10 的"自动 blend-out 在段尾前 BlendOut 时长触发"是两个独立机制**，见 §4.7 结束位纪律）。
