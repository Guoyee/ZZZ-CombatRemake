...
> **状态**: 实施中 —— Task 3/4 C++ 完成；**冲刺攻击/闪避反击 ✅（珂蕾妲资产完成、流程跑通，2026-09-02）**；**A 层窗口 GE 化已取消**（2026-08-29 定稿：窗口 tag 保持 LooseTag，C/B 层状态 GE 已落地）；**普通切换已落地（2026-09-02，见架构文档 §4.9.1）**；**特殊技（普通/强化 + 快速派生）+ 能量系统 C++ ✅ 2026-09-03**（详见架构文档 §4.13；资产待做）；**完美闪避慢动作发放点定稿 ✅ C++（2026-09-03）**：闪避按下只给敌人挂 `Effect.Enemy.Dodged`，敌人攻击蒙太奇伤害帧后的 EnemyDodgeSlow notify 消费并自施减速——慢放从打空后开始（资产待做：AM_EnemyAttack 摆 notify）；Assist（弹刀/突击）C++、切换自动判定、TeamPanel 待做
...
- **Task 3/3.5 收尾 + Task 4**：完美闪避判定前移（按下时窗口查询）、双减速（敌人 0.15 + 玩家 0.5 notify 驱动——**2026-09-03 敌人侧发放点改定**：敌人 0.15 由敌人蒙太奇消费 notify 在打空后发放，不再按下即放，见下文条目）、震屏 `GC_ZZZ_CameraShake`（含 R20 三处重写）、TimeDilation 桥接（敌人+玩家）、ApplyHitStop、敌人受击硬直（Staggered）、FindNearestEnemy 助手、`GC_ZZZ_DamageNumber` 补 R20。
...
- **2026-09-03 完美闪避慢动作发放点定稿（构建通过，C++ 完成）**：敌人攻击闪黄被完美闪避时，慢动作起点从"闪避按下瞬间"（敌人在前摇、攻击未出手）改到**伤害帧之后（打空后）**。机制：判定仍在按下时（黄闪窗查询，判定=闪避成功，无需伤害帧确认）→ `UZZZDodge` 只给敌人挂 **`Effect.Enemy.Dodged`**（A 层 LooseTag）→ 敌人攻击蒙太奇**伤害帧之后**摆新 notify **`UZZZAnimNotify_EnemyDodgeSlow`**（伤害帧 0.42s，建议 ≈0.5s）→ notify 查 tag → 移除（消费，单次攻击至多一次）→ 敌人自施减速 GE（**`UZZZGameplayEffect_SlowMotion`** C++ 载体：Duration 1.0s 世界时间 + TimeDilation 0.15，照 HitStop 载体模式；notify 槽默认它、可覆写既有 GE_SlowMotion BP）→ `UZZZEnemyAttack::EndAbility` 兜底清 tag（双轨②，被打断/未到 notify 不残留，防下次攻击误消费）。**玩家慢放不动**（GE_PlayerSlowMotion 0.5，玩家蒙太奇位移末段 DodgeSlowStart notify 驱动）。玩家侧 `SlowMotionEffect` 槽删除（GA_Dodge 资产旧配置被忽略，无碍）。⚠ **资产待做**：AM_EnemyAttack 在 0.42s 伤害帧后摆 EnemyDodgeSlow notify（须在攻击可播放段内——GA 结束前，否则 EndAbility 兜底先清 tag）。PIE：黄闪内闪避 → 攻击正常挥出 → 挥空瞬间敌人慢动作起 → 追击反击。
